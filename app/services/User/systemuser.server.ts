import { Response, ResponseType } from '~/utils/utils-server'
...
export const createSystemUser = async (
    ...
    try {
        if (clientId) {
            ...
            const isSysteUser = await canUser(
                userId,
                'create',
                'SystemUser',
                {}
...
export const updateSystemUser = async (
    ...
    try {
        if (clientId) {
            const isClientUser = await canUser(
                editorId,
                'update',
                'SystemUser',
                {
            ...
            if (isClientUser?.status === 200) {
                ...
                const editedUserClient = (await getUserEntities(
                    editorId
                )) as any
                if (editedUserClient?.data?.id === clientId) {
                    // create user
                    const response = await updateSystemUserDb(
                        userId,
                        data,
                        editorId
                    )
