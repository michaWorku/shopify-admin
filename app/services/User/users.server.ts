...
export const userFormHandler = async (request: Request, schema: z.ZodObject<any>) => {
  ...
  if (!!phone) {
    ...
    phone = `251${parsedPhone.startsWith("0")
      ? parsedPhone.slice(1)
...
export const getUsers = async (request: Request) => {
  try {
    ...
    if (typeof filters === "string" && !!filters) filters = JSON.parse(filters)
    ...
    const users = await db.user.findMany({
      ...
      orderBy: sort?.length ? (sort?.map((item: any) => {
        if (item.field === 'name') {
          return {
            firstName: item.sort
          }
      ...
      data: {
        users: users?.map((user: any) => ({
          ...
          updatedAt: moment(new Date(user?.updatedAt), format)
            .tz("Africa/Addis_Ababa")
            .format(format),
          deletedAt: !!user?.deltedAt ? moment(new Date(user?.deletedAt), format)
            .tz("Africa/Addis_Ababa")
            .format(format) : "",
        })),
      },
